  ...
  return (
    <Animated.View style={[styles.container, containerAnimatedStyle]}>
      ...
      {floatingItems.map((item) => (
        <FloatingItem
          key={`${item.startX}-${item.startY}-${item.delay}`}
          {...item}
        />
      ))}
      ...
      <View style={styles.dotsPattern}>
        {Array.from({ length: 5 }, (_, i) => 30 + i * 25).map((leftPos, i) => (
          <View
            key={`dot-left-${leftPos}`}
            style={[styles.dot, { left: leftPos, opacity: 0.2 + i * 0.1 }]}
          />
        ))}
      </View>
    </Animated.View>
